<template>
    <div class="chart">
        ...
        <ul class="navigator">
            <li @click="changePage(index)" :class="{'active':index == currentIndex}" class="navigatorItem" v-for="(item,index) in quanguoTrendChart" v-bind:key="index">{{item.title}}</li>
        </ul>
...
export default {
    ...
    methods:{
        changePage(index){//点击哪个就把index设置到 this.currentIndex上
